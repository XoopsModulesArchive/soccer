...
if($add_submit)
{
    ...
    $query = $xoopsDB->query("SELECT SeasonName FROM ".$xoopsDB->prefix("soccer_seasonnames")." WHERE SeasonName = $name");
    ...
    if($name != '')
    {
        $xoopsDB->query("INSERT INTO ".$xoopsDB->prefix("soccer_seasonnames")." SET
			SeasonName = $name,
			SeasonLine = '$drawline',
...
elseif($modify_submit)
{
    ...
    if($name != '')
    {
        ...
        if($defseason == 1)
        {
            $xoopsDB->query("UPDATE ".$xoopsDB->prefix("soccer_seasonnames")." SET
				SeasonDefault = '0'");
        }
        $xoopsDB->query("UPDATE ".$xoopsDB->prefix("soccer_seasonnames")." SET
			SeasonName = $name,
...
elseif($delete_submit)
{
    ...
    $query = $xoopsDB->query("SELECT M.LeagueMatchID
		FROM ".$xoopsDB->prefix("soccer_leaguematches")." M, ".$xoopsDB->prefix("soccer_seasonnames")." S
		WHERE M.LeagueMatchSeasonID = '$seasonid'");
    
    if($xoopsDB->getRowsNum($query) == 0)
    {
        $xoopsDB->query("DELETE FROM ".$xoopsDB->prefix("soccer_seasonnames")." WHERE SeasonID = '$seasonid'");
    }
    else
    {
...
?>
	...
	<table align="center" width="600">
		...
		elseif($action == 'modify')
		{
		    ...
		    $get_season = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("soccer_seasonnames")." WHERE SeasonID = '$seasonid' LIMIT 1");
	   ...
	   <tr>
		...
		<td align="left" valign="top">
		<?php
		$get_seasons = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("soccer_seasonnames")." ORDER BY SeasonName");
