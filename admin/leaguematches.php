...
elseif (!isset($_SESSION['season_id'])) {
    $sql = "SELECT SeasonID, SeasonName FROM ".$xoopsDB->prefix("soccer_seasonnames")." WHERE SeasonDefault=1";
...
//
$query = $xoopsDB->query("SELECT OpponentID FROM ".$xoopsDB->prefix("soccer_opponents"));
...
if($add_submit)
{
    ...
    while($i < 15)
    {
        ...
        $time_local = $_POST['time_local'];
        $time_yours = $_POST['time_yours'];        
        ...
        if($home[$i] != $away[$i])
        {
            ...
            $query = $xoopsDB->query("SELECT LM.LeagueMatchID FROM
				".$xoopsDB->prefix("soccer_leaguematches")." LM
				WHERE
            ...
            if($xoopsDB->getRowsNum($query) == 0)
            {
                if (($home_goals[$i] !== null) && ($home_goals[$i] !== null)) {
                    $xoopsDB->query("INSERT INTO ".$xoopsDB->prefix("soccer_leaguematches")." SET
						LeagueMatchSeasonID = '$seasonid',
						LeagueMatchDate = '$dateandtime',
						LeagueMatchTimeLocal = '$time_local',
						LeagueMatchTimeYours = '$time_yours',												
...
elseif($modifyall_submit)
{
    ...
    //
    $xoopsDB->query("DELETE FROM ".$xoopsDB->prefix("soccer_leaguematches")."
    ...
    while($i < $qty)
    {
        ...
        $time_local[$i] = $time_local[$i];
        $time_yours[$i] = $time_yours[$i]; 
        ...
        if (($home_goals[$i] !== null) && ($away_goals[$i] !== null)) {
            $xoopsDB->query("INSERT INTO ".$xoopsDB->prefix("soccer_leaguematches")." SET
				LeagueMatchSeasonID = '$seasonid',
				LeagueMatchDate = '$dateandtime',
				LeagueMatchTimeLocal = '$time_local[$i]',
				LeagueMatchTimeYours = '$time_yours[$i]',					
				LeagueMatchHomeID = '$home[$i]',
				LeagueMatchAwayID = '$away[$i]',
				LeagueMatchHomeWinnerID = '$home_winner',
				LeagueMatchHomeLoserID = '$home_loser',
...
elseif($modify_submit)
{
	...
	$time_local = $_POST['time_local']; //mb
	$time_yours = $_POST['time_yours']; //mb
    ...
    if($home != $away)
    {
        ...
        if($xoopsDB->getRowsNum($query) < 2)
        {
            if (($home_goals !== null) && ($away_goals !== null)) {
                $xoopsDB->query("UPDATE ".$xoopsDB->prefix("soccer_leaguematches")." SET
					LeagueMatchDate = '$dateandtime',
					LeagueMatchTimeLocal = '$time_local',
					LeagueMatchTimeYours = '$time_yours',									
...
{
    $mid = intval($_POST['mid']);
    $xoopsDB->query("DELETE FROM ".$xoopsDB->prefix("soccer_leaguematches")." WHERE LeagueMatchID = '$mid' LIMIT 1");
}
	...
	<table width="700">
		...
		<td align="left" valign="middle"><b><?php echo _AM_TIMELOCAL;?></b></td>
		<td align="left" valign="middle"><b><?php echo _AM_TIMEYOURS;?></b></td>
		...
		FROM ".$xoopsDB->prefix("soccer_opponents")."
		...
		while($i < 15)
		{
			...
			<td align=\"center\" valign=\"middle\"><input type=\"text\" name=\"time_local[$i]\" size=\"6\"></td>
			<td align=\"center\" valign=\"middle\"><input type=\"text\" name=\"time_yours[$i]\" size=\"6\"></td>
		...
		LM.LeagueMatchTimeLocal AS timelocal,
		LM.LeagueMatchTimeYours AS timeyours,	

		LM.LeagueMatchHomeGoals AS homegoals,
		LM.LeagueMatchAwayGoals AS awaygoals
		FROM ".$xoopsDB->prefix("soccer_leaguematches")." LM
		...
		while($matchdata = $xoopsDB->fetchArray($get_matches))
		{
			...
			<td align=\"center\" valign=\"middle\"><input type=\"text\" name=\"time_local[$i]\" size=\"6\" value=\"$matchdata[timelocal]\"></td>
			<td align=\"center\" valign=\"middle\"><input type=\"text\" name=\"time_yours[$i]\" size=\"6\" value=\"$matchdata[timeyours]\"></td>
		...
		<td align="center" valign="middle"><input type="text" name="time_local" size="6" value="<?= $matchdata['timelocal'] ?>"></td>
		<td align="center" valign="middle"><input type="text" name="time_yours" size="6" value="<?= $matchdata['timeyours'] ?>"></td>
		...
		LM.LeagueMatchTimeLocal AS time_local,
		LM.LeagueMatchTimeYours AS time_yours,
		...
		FROM ".$xoopsDB->prefix("soccer_leaguematches")." LM, ".$xoopsDB->prefix("soccer_opponents")." O, ".$xoopsDB->prefix("soccer_opponents")." OP
