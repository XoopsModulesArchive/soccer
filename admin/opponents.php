...
if($add_submit)
{
    ...
    $query = $xoopsDB->query("SELECT OpponentName FROM ".$xoopsDB->prefix("soccer_opponents")." WHERE OpponentName = $opponent");
    ...
    if($opponent != '')
    {
        $xoopsDB->query("INSERT INTO ".$xoopsDB->prefix("soccer_opponents")." SET OpponentName = $opponent");
...
elseif($modify_submit)
{
    ...
    if($opponent != '')
    {
        ...
        if($own == 1)
        {
            $xoopsDB->query("UPDATE ".$xoopsDB->prefix("soccer_opponents")." SET
				OpponentOwn = '0'
				WHERE OpponentOwn = '1'
				");
        }
...
elseif($delete_submit)
{
    ...
    $query = $xoopsDB->query("SELECT LeagueMatchID
		FROM ".$xoopsDB->prefix("soccer_leaguematches")."
		WHERE LeagueMatchHomeID = $opponentid OR LeagueMatchAwayID = $opponentid");
    
    if($xoopsDB->getRowsNum($query) == 0)
    {
        $xoopsDB->query("DELETE FROM ".$xoopsDB->prefix("soccer_opponents")." WHERE OpponentID = $opponentid");
...
elseif($d_points_add)
{
    ...
    if(is_numeric($d_points) && $d_points != '')
    {
        ...
        //Adds
        //
        $xoopsDB->query("INSERT INTO ".$xoopsDB->prefix("soccer_deductedpoints")." SET
			seasonid = $seasonid,
			teamid = $teamid,
			points = $d_points");
    }
...
elseif($d_points_modify)
{
    ...
    if(is_numeric($d_points) && $d_points != '')
    {
        ...
        if($d_points == 0)
        {
            $xoopsDB->query("DELETE FROM ".$xoopsDB->prefix("soccer_deductedpoints")."
				WHERE id = $id");
        }
        //
        ...
        {
            $xoopsDB->query("UPDATE ".$xoopsDB->prefix("soccer_deductedpoints")." SET
				points = $d_points
				WHERE id = $id");
        }
    }
...
?>
	...
	<table align="center" width="600">
		...
		elseif($action == 'modify')
		{
		    ...
		    $get_opponent = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("soccer_opponents")." WHERE OpponentID = $opponentid LIMIT 1");
		...
		<td align="left" valign="top">
		<?php
		$get_opponents = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("soccer_opponents")." ORDER BY OpponentName");
