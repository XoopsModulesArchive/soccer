...
//
$sql = "SELECT SeasonID FROM ".$xoopsDB->prefix("soccer_seasonnames")." WHERE SeasonDefault=1";
...
$updated_query = $xoopsDB->query("SELECT MAX(LeagueMatchCreated) AS last_updated FROM ".$xoopsDB->prefix("soccer_leaguematches"));
...
//
$get_seasons = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("soccer_seasonnames")." WHERE SeasonPublish = '1' ORDER BY SeasonName");
...
<img src="images/soccer1.jpg" alt=""> <b><font color="#FF0000" size="7"> &nbsp;2010 World Cup </font></b><img src="images/soccer2.jpg" alt=""><br><br>
</p>
...
if($defaulttable == 1)
{
    echo"<option value=\"4\" SELECTED>"._LS_MODETABLESIMP."</option>
	<option value=\"1\">"._LS_MODETABLETRA."</option>
...
<td>
	...
	<td bgcolor="<?php echo $inside_c ?>" align="center">
		...
		O.OpponentID AS id
		FROM ".$xoopsDB->prefix("soccer_opponents")." O, ".$xoopsDB->prefix("soccer_leaguematches")." LM
		...
		while($data = $xoopsDB->fetchArray($get_teams))
		{
		    ...
		    if($defaulttable == 1 || $defaulttable == 3 || $defaulttable == 4)
		    {
		        ...
		        //
		        //Lets make change in points if there are data in soccer_deductedpoints-table
		        //
		        if($defaulttable == 1 || $defaulttable == 4)
		        {
		            $get_deduct = $xoopsDB->query("SELECT points
					FROM ".$xoopsDB->prefix("soccer_deductedpoints")." 
       ...
       <tr>
		...
		else
		{
		    ...
		    while($data = $xoopsDB->fetchArray($get_matches))
		    {
				...
				<td style=\"padding-left:5px;\" align=\"left\" valign=\"top\">
                <i>Time: $data[timelocal] (USA EST: $data[timeyours])				
				</td>
				<td align=\"left\" valign=\"top\">";
